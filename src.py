...
# Credenciais de login mestre (não são as senhas do banco de dados,
# são as credenciais da tela de login do próprio dashboard).
MASTER_USERNAME = os.environ['MASTER_USERNAME']
MASTER_PASSWORD = os.environ['MASTER_PASSWORD']
COMPRAS_USERNAME = os.environ['COMPRAS_USERNAME']
COMPRAS_PASSWORD = os.environ['COMPRAS_PASSWORD']
...
#   CCUSTO_PASSWORDS='{"1011": "xxxx", "1012": "xxxx", ...}'
#
# Ou aponte CCUSTO_PASSWORDS_FILE para um arquivo .json local (recomendado
# para dicionários grandes como este).
ccusto_passwords_file = os.environ.get('CCUSTO_PASSWORDS_FILE')
if ccusto_passwords_file:
    with open(ccusto_passwords_file, 'r', encoding='utf-8') as f:
        ccusto_passwords = json.load(f)
else:
    ccusto_passwords = json.loads(os.environ['CCUSTO_PASSWORDS'])
...
def login(n_clicks, username, password):
    ...
    if ((username == MASTER_USERNAME and password == MASTER_PASSWORD) or
    (username == COMPRAS_USERNAME and password == COMPRAS_PASSWORD)):
